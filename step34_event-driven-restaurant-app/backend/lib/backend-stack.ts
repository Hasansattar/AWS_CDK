...
export class BackendStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    // The code that defines your stack goes here
     ...
     //////////////////////// creating User Pool /////////////////////////////////
     //***for signin and signup users, we need to create user pool***
    ...
    //***Create Client for User Account, so we can use userpool in the frontend using amplify***

    const userPoolClient = new cognito.UserPoolClient(this, "userPoolClient-Amplify", {
      userPool,
    })

    // create group for Admin , all admins account lies in this group
    ...
    // In Appsync API,we used authorrization type is userPool , only those can access api who has authorized
    // Appsync API
    const api = new appsync.GraphqlApi(this, "Api", {
      name: "appsyncEventbridgeAPI",
      schema: appsync.SchemaFile.fromAsset("utils/schema.gql"),
      authorizationConfig: {
    ...
    const restaurantTimeSlotsDS = api.addDynamoDbDataSource('RestaurantTimeSlots', RestaurantAppTable);
      ...
      /* Query */
      restaurantTimeSlotsDS.createResolver("createResolverQuerygetTimeSlots",{
        typeName: "Query",
    ...
    const appsyncNoDS = api.addNoneDataSource("noDataSource", {
      ...
      //we used HttpDataSource for Api to send data into event bridge
    ...
    mutations.forEach((mut) => {
      ...
      httpEventTriggerDS.createResolver(`createResolverMutationmut${randomBytes(16).toString("hex")}`,{
        typeName: "Mutation",
        fieldName: mut,
    ...
    appsyncNoDS.createResolver("createResolverMutationgenerateAction",{
      typeName: "Mutation",
