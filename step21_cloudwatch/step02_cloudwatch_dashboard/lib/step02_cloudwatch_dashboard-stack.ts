...
export class Step02CloudwatchDashboardStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    let api = new apigw.HttpApi(this, 'HttpAPI', {
      defaultIntegration: new integrations.HttpLambdaIntegration("HttpLambdaIntegration",dynamoLambda)  
    });
    ...
    new cloudwatch.Dashboard(this, 'CloudWatchDashBoard').addWidgets(
      ...
      this.buildGraphWidget('DynamoDB Latency', [
        table.metricSuccessfulRequestLatency({ dimensionsMap: { "TableName": table.tableName, "Operation": "GetItem" } }),
        table.metricSuccessfulRequestLatency({ dimensionsMap: { "TableName": table.tableName, "Operation": "UpdateItem" } }),
        table.metricSuccessfulRequestLatency({ dimensionsMap: { "TableName": table.tableName, "Operation": "PutItem" } }),
        table.metricSuccessfulRequestLatency({ dimensionsMap: { "TableName": table.tableName, "Operation": "DeleteItem" } }),
        table.metricSuccessfulRequestLatency({ dimensionsMap: { "TableName": table.tableName, "Operation": "Query" } }),
      ], true),
